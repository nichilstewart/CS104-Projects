...
class CompanyTracker {
 ...
 private:
  ...
  Company* findParent(int i);
  /* Finds the parent company for the 'i'th student in companies array
     and returns pointer to function call. */

  bool invalidInput(int i);
  bool invalidInput(int i, int j);
  /* Makes sure the inputs are in bounds of the array 
     and not equal to each other. */

  void DeleteHelper(Company* del_company);
  /* Helps delete companies using recursive function. 
     First finds the parent company, creates copies of 
     its children. Then sets their parent pointers to 
     null, and then deletes the parent. Then calls 
     itself recursively with those 2 children as 
     arguments. */
